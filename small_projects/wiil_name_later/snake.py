...
class App:
    ...
    def get_observations(self):
        info="Doing great"
        obs=[1,self.player.x[-1],self.player.y[-1],self.apple.x,self.apple.y,self.player.length,self.windowHeight,self.windowWidth]
        return [obs,self.player.length+self.player.survive_time,not self.game_state,info]

def run(actions):
     env=App() 
     env.on_execute()
     brain=Agents(gamma=0.99,epsilon=0,batch_size=64,n_actions=4,input_dims=[8],alpha=0.03)
     scores=[]
     eps_history=[]
     n_games=500
     score=0
